...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.container and not insect.container and not self.ant.ant:
                    self.ant.ant = insect
                elif not self.ant.container and insect.container and not insect.ant:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if insect.name == 'Queen' and insect.queens_truth:
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant:
            return self.place.ant.blocked_path is True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        position = self.place
        bee_position = 0
        while position.entrance:
            if not position.bees or not (self.min_range <= bee_position and bee_position <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            for bees in copy:
                bees.armor -= self.damage
                if bees.armor <= 0:
                    self.place.remove_insect(bees)
            self.place.remove_insect(self)
        # END Problem 5
...
# The WallAnt class
class WallAnt(Ant):
    """WalAnt has high defense and is super lame and boring"""
    name = 'Wall'
    implemented = True
    food_cost = 4
    def __init__(self):
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting > 0 or not self.place.bees:
            self.digesting -= 1
        else:
...
# END Problem 13
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        self.armor = 1
        self.queens_truth = not self.queens_truth
        self.ants_behind = self.ants_behind
        QueenAnt.queens_truth += 1
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.queens_truth:
            self.throw_at(self.nearest_bee(colony.hive))
            position = self.place.exit
            while position:
                if position.ant and position.ant not in self.ants_behind:
                    position.ant.damage *= 2
                    self.ants_behind += [position.ant]
                if position.ant and position.ant.container and position.ant.ant:
                    if position.ant.ant not in self.ants_behind:
                        position.ant.ant.damage *= 2
                        self.ants_behind += [position.ant.ant]
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    new_action,condition = effect(Bee.action)
    dur = duration
    if dur > 2:
        for i in range(2):
            bee.effects = [condition()] + bee.effects
    else:
        for i in range(duration):
            bee.effects = [condition()] + bee.effects
    def applied(colony):
        nonlocal dur,new_action,condition
        new_action(bee,colony)
        if len(bee.effect) > 1:
            if bee.effects[0] == 'slow':
                eff = make_slow
                new_action,ignore = eff(Bee.action)
            elif bee.effects[0] == 'stun':
                eff = make_stun
                new_action,ignore = eff(Bee.action)
        if len(bee.effects) > 0:
            del bee.effects[0]
        #print(bee.effects)
        if dur > 0:
            dur -= 1
